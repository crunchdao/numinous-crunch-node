...
async def main():
    ...
    if cfg.mrc_secure_credentials_dir:
        secure_credentials = SecureCredentials.from_directory(cfg.mrc_secure_credentials_dir)

    concurrent_runner = DynamicSubclassModelConcurrentRunner(
        timeout=cfg.run_models_timeout,
        crunch_id=cfg.mrc_crunch_id,
        host=cfg.mrc_host,
        port=cfg.mrc_port,
        base_classname=cfg.mrc_base_classname,
    ...
    pull_events_task = PullEvents(
        interval_seconds=cfg.pull_events_interval,
        page_size=50,
    ...
    resolve_events_task = ResolveEvents(
        interval_seconds=cfg.resolve_events_interval,
        db_operations=db_operations,
        api_client=api_client,
        page_size=100,
        logger=logger,
    ...
    delete_events_task = DeleteEvents(
        interval_seconds=cfg.delete_events_interval,
        db_operations=db_operations,
        api_client=api_client,
        page_size=100,
        logger=logger,
    ...
    register_models_task = RegisterModels(
        interval_seconds=cfg.register_models_interval,
        db_operations=db_operations,
    ...
    run_models_task = RunModels(
        interval_seconds=cfg.run_models_interval,
        db_operations=db_operations,
    ...
    scoring_task = CrunchNodeScoring(
        interval_seconds=cfg.scoring_interval,
        db_operations=db_operations,
        logger=logger,
        page_size=cfg.scoring_page_size,
    )

    export_scores_task = ExportScoresPg(
        interval_seconds=cfg.export_scores_interval,
        page_size=cfg.export_batch_size,
        db_operations=db_operations,
        pg_client=pg_client,
        logger=logger,
        crunch_node_uid=cfg.crunch_node_uid,
        crunch_node_hotkey=cfg.crunch_node_hotkey,
    )

    export_predictions_task = ExportPredictionsPg(
        interval_seconds=cfg.export_predictions_interval,
        db_operations=db_operations,
        pg_client=pg_client,
        batch_size=min(cfg.export_batch_size, 300),
        crunch_node_uid=cfg.crunch_node_uid,
        crunch_node_hotkey=cfg.crunch_node_hotkey,
        logger=logger,
    )
    ...
    export_agent_runs_task = ExportAgentRunsPg(
        interval_seconds=cfg.export_agent_runs_interval,
    ...
    export_agent_run_logs_task = ExportAgentRunLogsPg(
        interval_seconds=cfg.export_agent_run_logs_interval,
        batch_size=cfg.export_batch_size,
        db_operations=db_operations,
        pg_client=pg_client,
        logger=logger,
    ...
    db_cleaner_task = DbCleaner(
        interval_seconds=cfg.db_cleaner_interval,
        db_operations=db_operations,
        batch_size=4000,
        logger=logger,
    )
    ...
    vacuum_task = DbVacuum(
        interval_seconds=cfg.db_vacuum_interval,
        db_operations=db_operations,
        logger=logger,
        pages=500,
    )
    ...
    logger.info(
        "Crunch node started",
        extra={
            "numinous_env": cfg.numinous_env,
            "database_path": cfg.database_path,
            "pg_dsn": cfg.pg_dsn.split("@")[-1] if "@" in cfg.pg_dsn else "***",
            "python": sys.version,
